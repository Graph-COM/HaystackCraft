    ...
    if is_final_round:
        ...
        if len(all_summaries) == 0:
            ...
            for i, summary in enumerate(all_summaries):
                prev_summary_text += f"Round {i+1}: {summary}\n"
            prev_summary_text += "\n"
        instruction_text = "Based on your previous analyses and the potentially new articles provided, decide if you are confident in answering the question or you need additional information."
...
def get_pred_multi(data, args, out_file):
    ...
    template = open('prompts/dynamic_multi_round_context.txt', encoding='utf-8').read()
    ...
    for item in tqdm(data):
        ...
        round_idx = 0
        early_stop = False
        while round_idx < args.max_rounds - 1:
...
def main(args):
    ...
    if args.ppr:
        ...
        save_dir = f"max_{args.max_rounds}_round_results/{args.retriever}_ppr/seed_{args.k}_alpha_{args.alpha}/{args.llm}/{args.context_size}/{args.order}"
    else:
        save_dir = f"max_{args.max_rounds}_round_results/{args.retriever}/{args.llm}/{args.context_size}/{args.order}"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_rounds", type=int, required=True, choices=[1, 2, 3])
